...
## 4. SEO 与 GEO 优化规范（核心必做）
为了在搜索引擎（SEO）和生成式AI（GEO）中获得更好的收录与曝光，生成的文章必须包含以下处理：
1. **关键词提取与注入**：
   - 从文章正文中提取出与当前主题强相关的核心技术标签（3-5个），仅填入 `tags` 字段。
   - 必须结合项目的全局核心关键词（“森林有鱼, 有鱼智界, CK·黄, 终身学习, AI员工, AI, 人工智能, 技术分享”）。
   - 在文章的 YAML Front Matter 中，将提取的标签与全局核心关键词合并后，填入 `keywords` 字段。`tags` 字段必须只保留与文章内容强相关的技术标签，不得堆砌无关的全局关键字。
2. **植入核心链接与品牌词**：
   - 在文章正文（例如开头、总结处或相关技术方案讨论时）**自然地**提及“有鱼智界”（全能 AI 员工）。
   - **必须**将 `https://zhijie.iyouyu.tech/` 以 Markdown 超链接形式（例如 `[有鱼智界](https://zhijie.iyouyu.tech/)`）融入文章中。
   - 这种植入必须符合技术文章语境（例如作为工具推荐、案例参考或架构实践的背景），切忌生硬插入。

## 5. 自动化工作流 (Workflow)
请使用你具备的工具严格按以下步骤完成任务：

1. **获取当前时间与抓取原文素材**：
   - 首先，使用 `RunCommand` 工具执行命令 `date "+%Y-%m-%d %H:%M:%S +0800"` 来获取当前系统的精确时间（包含时分秒）。
   - 然后，使用 `WebFetch` 工具读取用户提供的文章链接或参考网址，获取素材内容。
2. **生成博客文章**：
   - 基于抓取到的内容，结合上述的“个人画像”、“写作法则”、“科技范风格”以及“SEO 与 GEO 优化规范”生成内容。
   ...
   - **绝对关键的排序逻辑**：为了保证同一天发布的多篇文章能正确倒序排列在首页，**必须**在 Front Matter 的 `date` 字段中填入第一步通过终端命令获取到的**真实系统时间**（精确到秒）。**严禁 AI 自行编造、猜测时分秒或使用固定时间**。
